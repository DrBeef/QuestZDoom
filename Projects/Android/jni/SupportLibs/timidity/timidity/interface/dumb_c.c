...
static int ctl_open(int using_stdin, int using_stdout)
{
	//__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "deer god");
  if(using_stdout)
    outfp=stderr;
  else
    outfp=stdout;
...
static void ctl_close(void)
{
	//__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "ac");
  fflush(outfp);
  ctl.opened=0;
}
...
static int ctl_read(int32 *valp)
{
	//__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "yolo");
  return RC_NONE;
}
...
static int cmsg(int type, int verbosity_level, char *fmt, ...)
{
  ...
  if (!ctl.opened)
  ...
  else
    {
      ...
      fputs(NLS, outfp);
	  __android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", buffer);
      fflush(outfp);
    }
  va_end(ap);
  return 0;
...
static void ctl_total_time(long tt)
{
	__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "timin");
  int mins, secs;
...
static void ctl_file_name(char *name)
{
	//__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "fill nam");
...
{
	//__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "mor timin");
  int mins;
...
static void ctl_lyric(int lyricid)
{
	//__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "yolocaust");
    char *lyric;
