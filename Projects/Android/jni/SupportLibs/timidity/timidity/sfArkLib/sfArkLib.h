...
#define sfkl_msg(MessageText, Flags) __android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "%s", MessageText)
#define sfkl_UpdateProgress(p)
#define sfkl_DisplayNotes(NotesText, NotesFileName);
...
//extern int sfkl_GetLicenseAgreement(const char *LicenseText, const char *LicenseFileName); // Display/confirm license
//extern void sfkl_DisplayNotes(const char *NotesText, const char *NotesFileName);	// Display notes text file
#define sfkl_GetLicenseAgreement(LicenseText, LicenseFileName) 1
// Functions in sfArkLib for use by Application...
extern "C" unsigned short	sfkl_GetVersion(void);
extern "C" int 		skfl_Decode(const char *InFileName, const char *ReqOutFileName);
