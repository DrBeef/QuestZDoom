...
void midi_program_change(int ch, int prog)
{
	...
	int newbank, b, p, map;
	if(ch&0x8000) // Clear the instrument lock. There should never be more than 0xFF channels right?
	{
	...
	if((channel[ch&0x7FF].forced_instrument&&(ch&0x800))||(!channel[ch&0x7FF].forced_instrument&&!(ch&0x800))||prescanning_flag)
	{
		ch&=0x7FF;
...
{
	if(ch&0x8000) // Clear the instrument lock. There should never be more than 0xFF channels right? Heck, 32 should be the max
	{
	...
	if((channel[ch&0x7FF].forced_volume&&(ch&0x800))||(!channel[ch&0x7FF].forced_volume&&!(ch&0x800))||prescanning_flag)
	{
