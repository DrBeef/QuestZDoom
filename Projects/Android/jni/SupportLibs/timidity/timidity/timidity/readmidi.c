...
#define MIDIEVENT(at, t, ch, pa, pb) \
    { MidiEvent event; SETMIDIEVENT(event, at, t, ch, pa, pb); \
      if(!readmidi_error_flag) readmidi_add_event(&event); }
...
void readmidi_add_ctl_event(int32 at, int ch, int a, int b)
{
    ...
    if(convert_midi_control_change(ch, a, b, &ev)&&!readmidi_error_flag)
    {
	ev.time = at;
			    ...
			    struct timidity_file *tf)
{
    ...
    if(parse_sysex_event(val, len, &ev)&&!readmidi_error_flag)
    {
	ev.time = at;
	readmidi_add_event(&ev);
    }
    ...
    {
	for (i = 0; (!readmidi_error_flag&&i < ne); i++) {
	    evm[i].time = at;
