...
Voice_SetVolume (UBYTE v, UWORD vol)
{
  ...
  if (ModV[v].vol != vol&&!readmidi_error_flag) {
    ModV[v].vol = vol;
...
/* EAW -- I think this is only here for TRACE_SLIDE_NOTES support? */
  ...
  if (ModV[v].wheel != bend&&!readmidi_error_flag)
    {
      ModV[v].wheel = bend;
...
Voice_SetPanning (UBYTE v, ULONG pan)
{
  ...
  if (pan != ModV[v].pan&&!readmidi_error_flag) {
    ModV[v].pan = pan;
...
Voice_Play (UBYTE v, SAMPLE * s, ULONG start)
{
  ...
  if (ModV[v].sample != s->id&&!readmidi_error_flag)
    {
      ModV[v].sample = s->id;
      MIDIEVENT(at, ME_SET_PATCH, v, ModV[v].sample, 0);
    }

  if (start > 0&&!readmidi_error_flag)
    {
      int a, b;
      a = (start & 0xff);
      b = ((start >> 8) & 0xff);
