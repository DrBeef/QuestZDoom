...
static char const * auto_detect_format(sox_format_t * ft, char const * ext)
{
  ...
  CHECK(aac   , 0, 0, ""     , 0,  2, "\xff\xf1") // MPEG-4 aac
  CHECK(aac   , 0, 0, ""     , 0,  2, "\xff\xf9") // MPEG-2 aac
  CHECK(m4a   , 0, 0, ""     , 4,  4, "ftyp") // MP4-container aac?
...
static sox_encodings_info_t const s_sox_encodings_info[] = {
  ...
  {sox_encodings_lossy2, "MPEG-4 audio" , "MPEG-4 audio"},
 ...
 * In theory, we no longer need to check _NEWLIB_VERSION or __APPLE__ */
  ...
  // Android, no need to deal with pipes in our case
...
int sox_seek(sox_format_t * ft, sox_uint64_t offset, int whence)
{
	...
	ft->olength = (offset-ft->signal.rate)*ft->signal.channels;
